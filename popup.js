document.addEventListener('DOMContentLoaded', () => {
  ...
  const removeIconPaths = [
      { d: "M8 11a1 1 0 1 0 0 2h8a1 1 0 1 0 0-2z" },
      { d: "M1 5a4 4 0 0 1 4-4h14a4 4 0 0 1 4 4v14a4 4 0 0 1-4 4H5a4 4 0 0 1-4-4zm4-2h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2", "fill-rule": "evenodd", "clip-rule": "evenodd" }
  ];
  ...
  // Load bookmarks and theme preference
  loadBookmarks();
  loadThemePreference();
  ...
  function createSVGIcon(paths) {
      ...
      paths.forEach((pathData) => {
          ...
          pathElem.setAttributeNS(null, "d", pathData.d);
          if (pathData["fill-rule"]) {
              pathElem.setAttributeNS(null, "fill-rule", pathData["fill-rule"]);
          }
          if (pathData["clip-rule"]) {
              pathElem.setAttributeNS(null, "clip-rule", pathData["clip-rule"]);
          }
  ...
  function renderBookmark(bookmark) {
      ...
      const removeIcon = createSVGIcon(removeIconPaths);
      removeBtn.appendChild(removeIcon);
      removeBtn.addEventListener('click', () => removeBookmark(bookmark, li));
      actions.appendChild(removeBtn);

      const archiveBtn = document.createElement('button');
      const archiveIcon = createSVGIcon(archiveIconPaths);
  ...
  async function toggleTheme() {
      try {
          ...
          while (themeToggleButton.firstChild) {
              themeToggleButton.removeChild(themeToggleButton.firstChild);
          }

          // Update the button icon
          if (body.classList.contains('dark-mode')) {
              const icon = createSVGIcon(lightModeIconPaths);
              themeToggleButton.appendChild(icon);
          } else {
              const icon = createSVGIcon(darkModeIconPaths);
              themeToggleButton.appendChild(icon);
          }
